var config = {
    ...
    'THRESHOLD_SHOW_LABEL': 2,
    'THRESHOLD_COMMUNITY_SIZE': 5,
    'THRESHOLD_COMMUNITY_MAX_SIZE': 30,
    'MODE_SHOW_LABEL': 0,
    'MODE_SHOW_LINK': [true, true, true], // org, social, follow
    'THRESHOLD_NODE': 0,
    'THRESHOLD_DEGREE': 0,
    'CHART_TYPE': 'force',
    'GROUP_BY_FIELD': 'functionalarea',
    'HIDDEN_LEGEND': {},
    'CANVAS_WIDTH': 1600,
    'CANVAS_HEIGHT': 800,
    'CANVAS_BLANK': 150,
    ...
    'DICT': {
        'TYPE': ['g', 'n', 'f', 'c'],
        'FIELD': {
            'Geographic': ['boardarea', 'functionalarea', 'region', 'city', 'costcenter', 'gender'],
            'Category Burst': ['boardarea', 'functionalarea', 'region', 'city', 'costcenter', 'gender'],
            'Force': ['boardarea', 'functionalarea', 'region', 'city', 'costcenter', 'gender'],
            'Circular': ['boardarea', 'functionalarea', 'region', 'city', 'costcenter', 'gender']
        },
        'GEO_CITY': {},
        'GEO_CITY_DATA': {}
    },
    'DATA_SOURCE': {},
    'DATA_SOURCE_META': {},
    'DATA_SOURCE_FILE': '../data/peoplenetwork-I345795-10-0.json',
    // 'DATA_TIMESTAMP': null,
    'USER': '',
    'FOLLOW': false,
    'DEBUG': true,
    'LEVEL': [4, 3, 2], // level, s_level, f_level
    'API_SERVER_PATH': 'http://10.58.78.253:8001/pn/',
    'CACHE': false
};
...
function init_chart(topic, chart, debug) {
    ...
    if (querystring('user') != undefined) {
        config.USER = querystring('user');
    } else {
        config.USER = topic.toLowerCase();
    }

    if (querystring('follow') != undefined && querystring('follow').toLowerCase() == "true") {
        config.FOLLOW = true;
    }

    if (querystring('lv') != undefined && !isNaN(parseInt(querystring('lv')))) {
        if (parseInt(querystring('lv')) >= 0) {
            config.LEVEL[0] = parseInt(querystring('lv'));
        }
    }

    if (querystring('slv') != undefined && !isNaN(parseInt(querystring('slv')))) {
        if (parseInt(querystring('slv')) >= 0) {
            config.LEVEL[1] = parseInt(querystring('slv'));
        }
    }

    if (querystring('flv') != undefined && !isNaN(parseInt(querystring('flv')))) {
        if (parseInt(querystring('flv')) >= 0) {
            config.LEVEL[2] = parseInt(querystring('flv'));
        }
    }
    ...
    $.when($.getJSON("../data/map-city-geo.json"), $.getJSON("../data/cache-meta-data.json"))
        .done(function (map, meta) {
            if (map[1] === "success" && meta[1] === "success") {
                ...
                var filepath = config.API_SERVER_PATH + config.USER;

                if (config.LEVEL[0] > 0) {
                    filepath += ("?level=" + config.LEVEL[0]);
                } else {
                    alert("invalid value of level");
                }

                if (config.LEVEL[1] > 0) {
                    filepath += ("&slevel=" + config.LEVEL[1]);
                }

                if (config.FOLLOW) {
                    filepath += ("&follow=true");
                    if (config.LEVEL[2] > 0) {
                        filepath += ("&flevel=" + config.LEVEL[2]);
                    }
                }

                // for local version to visualize the json data source
                if (config.CACHE) {
                    filepath = config.DATA_SOURCE_FILE;
                } else {
...
function bind_select2(dataset, select2, user) {
    if (config.DEBUG) {
        select2.empty();
    ...
    $("h1#lbl-domain").html("User: " + user + " (" + dataset.nodes.length + ")");
}

function calculate_arrow_mode(c_mode, f_mode) {
    var mode = -1;
    if (c_mode >= 0 && f_mode >= 0) {
        if (c_mode == f_mode == 0) {
            mode = 0;
        } else if (c_mode == f_mode == 1) {
            mode = 1;
        } else {
            mode = 2;
        }
...
function generate_dataset(dataset) {
    ...
    nodes.forEach(function (node) {
        ...
        node.symbolSize = (node.value + 0.5) * 4;
    ...
    // 设置筛选边(edge)
    // 组织架构为虚线，opacity低，作为背景，无方向
    // 社交关系为实线，opacity高，作为主要内容，有方向，有颜色
    var links = [];
    dataset.links.forEach(function (edge) {
        if (edge.role == 'org') {
            ...
            EDGE_CURVENESS = 0;
            EDGE_TYPE = 'dashed';
            EDGE_SYMBOL_SIZE = [0, 3];
            EDGE_OPACITY = config.MODE_SHOW_LINK[0] ? 0.4 : 0;
            EDGE_COLOR = 'source';
        } else if (edge.role == 'social') {
            if (config.MODE_SHOW_LINK[1] && edge.c_mode >= 0 || config.MODE_SHOW_LINK[2] && edge.f_mode >= 0) {
                EDGE_CURVENESS = 0.2;
                EDGE_TYPE = 'solid';
                EDGE_OPACITY = 0.8;

                mode = calculate_arrow_mode(edge.c_mode, edge.f_mode);
                if (mode == 2) {
                    EDGE_SYMBOL_SIZE = [5, 5];
                } else if (mode == 1) {
                    EDGE_SYMBOL_SIZE = [0, 5];
                } else if (mode == 0) {
                    EDGE_SYMBOL_SIZE = [5, 0];
                } else {
                    EDGE_SYMBOL_SIZE = 0;
                }

                if (edge.c_mode >= 0 && edge.f_mode >= 0) {
                    EDGE_COLOR = '#96318a';
                    EDGE_WIDTH = 2;
                } else if (edge.c_mode < 0 && edge.f_mode >= 0) {
                    EDGE_COLOR = '#73c2c4';
                    EDGE_WIDTH = 0.5;
                } else if (edge.c_mode >= 0 && edge.f_mode <= 0) {
                    EDGE_COLOR = '#e69278';
        ...
        edge.symbolSize = EDGE_SYMBOL_SIZE;
        edge.lineStyle = {
            normal: {
                width: EDGE_WIDTH,
            ...
            tooltip: {
                formatter: function (param) {
                    if (param.dataType == 'edge') {
                        if (param.data.role == 'org') {
                            return config.DEBUG ? param.name + " :org" : " :org";
                        } else {
                            text = ['c: ', param.data.c_mode, ',f: ', param.data.f_mode, ',w: ', param.data.weight].join('');
                            return config.DEBUG ? param.name + " as " + text : text;
                        }
...
function bind_buttons(chart) {
    ...
    $("button#btn-refresh").click(function () {
        ...
        bind_select2(ds, $("#ddl-search"), config.USER);
    ...
    //     var filepath = config.API_SERVER_PATH + config.USER + "?nonorg=true";
    ...
    $("button#btn-download").on("click", function () {
        ...
        $("<a />", {
                "download": "pn-".concat(config.USER.toLowerCase(),
    ...
    $("#range-node").ionRangeSlider({
        ...
        step: 1,
        onFinish: function (obj) {
            config.THRESHOLD_NODE = config.LEVEL[0] - obj.from;
    ...
    $(".cbl-links-pattern>input:checkbox").click(function () {
        $(".cbl-links-pattern>input:checkbox").each(function (i, entry) {
            config.MODE_SHOW_LINK[i] = $(entry).is(':checked');
        });
